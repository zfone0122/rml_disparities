...
*description: Event-Study Analyses of RMLs and Race-Specific Property and Violent Crime Arrests, 
*             TWFE Estimates
**********************************************************
clear all 
...
**Panel A: Property Arrests 
	*c1 - white 
	reghdfe rate_property_1899 $lea $mml_mdl $police_econ $drug_pol $sw_pol F* L* [pw=pop_1899] if race==3, a(fips year) vce(cl fips)
		est store c1w
		
	*c1 - black 
	reghdfe rate_property_1899 $lea $mml_mdl $police_econ $drug_pol $sw_pol F* L* [pw=pop_1899] if race==2, a(fips year) vce(cl fips)
		est store c1b
	...
	graph save "Figure 3_property.gph", replace     
	graph export "Figure 3_property.png", replace     
	est clear 
	
**Panel B: Violent Arrests 
	*c1 - white 
	reghdfe rate_violent_1899 $lea $mml_mdl $police_econ $drug_pol $sw_pol F* L* [pw=pop_1899] if race==3, a(fips year) vce(cl fips)
		est store c1w
		
	*c1 - black 
	reghdfe rate_violent_1899 $lea $mml_mdl $police_econ $drug_pol $sw_pol F* L* [pw=pop_1899] if race==2, a(fips year) vce(cl fips)
		est store c1b
	...
	xtitle(Years Relative to RML Enactment, size(small)) ///
	ytitle(Coefficient Estimate, size(small)) ///
	yline(0, lcolor(gs8)) ylabel(-300(100)300,labsize(small))
	graph save "Figure 3_violent.gph", replace     
	graph export "Figure 3_violent.png", replace     
	est clear 
