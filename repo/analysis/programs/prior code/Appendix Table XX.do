...
gen ln_rate_total_nonmj_1899=ln(rate_total_nonmj_1899)
...
esttab c1 c2 c3 c4 using "Appendix Table XX.csv", label ///
	title("Appendix Table XX - Panel 1 - levels")  ///
	keep(rml rml_black) ///
...
***Panel 2: log-level 
*c1
reghdfe ln_rate_total_cannabis_1899 $sat_interx i.fips#i.black#c.t1 rml rml_black [pw=pop_1899], a(i.fips#i.black i.year#i.black) vce(cl fips)
	estadd local trends="Yes"
	estadd local reg_year="No"
	est store c1
 
*c2
reghdfe ln_rate_total_cannabis_1899 $sat_interx i.fips#i.black#c.t1 rml rml_black [pw=pop_1899], a(i.fips#i.black i.year#i.black i.year#i.black#i.region) vce(cl fips)
	estadd local trends="Yes"
	estadd local reg_year="Yes"
	est store c2
	
*c3
reghdfe ln_rate_total_nonmj_1899 $sat_interx i.fips#i.black#c.t1 rml rml_black [pw=pop_1899], a(i.fips#i.black i.year#i.black) vce(cl fips)
	estadd local trends="Yes"
	estadd local reg_year="No"
	est store c3
		
*c4
reghdfe ln_rate_total_nonmj_1899 $sat_interx i.fips#i.black#c.t1 rml rml_black [pw=pop_1899], a(i.fips#i.black i.year#i.black i.year#i.black#i.region) vce(cl fips)
	estadd local trends="Yes"
	estadd local reg_year="Yes"	
	est store c4	

esttab c1 c2 c3 c4 using "Appendix Table XX.csv", label ///
	title("Appendix Table XX - Panel 2 - log-level")  ///
	keep(rml rml_black) ///
	noobs scalars("N N" "trends State-specific linear time trends" "reg_year Census region-year FE") bfmt(3) sefmt(3) sfmt(0) star(* 0.10 ** 0.05 *** 0.01) ///
	mtitles("Marijuana Arrests" "" "Non-Marijuana Drug Arrests" "") nogaps se nonotes ///
	page append 
	est clear
