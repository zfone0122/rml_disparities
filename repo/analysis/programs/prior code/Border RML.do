...
******leads/lags for rml_state_border
use `rml_border', clear

keep year fips rml_state_border 

xtset fips year 
bysort fips: gen rml_state_border_1=d1.rml_state_border //first difference of policy variable to create events

foreach v in rml_state_border_1 {
...
drop rml_state_border_1 
ren (F*_1 L*_1) (F* L*)

*label the event variables for graphing purposes
label var F5_rml_state_border "-5+"
label var F4_rml_state_border "-4"
label var F3_rml_state_border "-3"
label var F2_rml_state_border "-2"
label var F1_rml_state_border "-1"
label var L0_rml_state_border "0"
label var L1_rml_state_border "1"
label var L2_rml_state_border "2"
label var L3_rml_state_border "3+"

*normalize first lead to zero (reference period)
replace F1_rml_state_border=0
...
gen black=race==2
foreach i in num_agencies_report unemployment pc_income prop_black prop_hisp democrat mml decrim beer_tax samaritan_alc samaritan_drug naloxone pdmp cig_tax ecigtax lnpolice_percapita eitc snap4 minimum_wage acaexp shall_law stand_ground lnmw lnbeer lnpci rml rml_state_border {
...
esttab c1 c2 c3 c4 c5 c6 c7 c8 using "Border RML.csv", label ///
		title("Border RML - Panel 1 ")  ///
		keep(rml) ///
...
/* Other Estimates 
**Panel IV: RML + Border State RML + drop states that border an RML but don't have an RML 
/* NOTE:

In this sample/specification, RML captures effect for 
RML states without a border. "RML + Border State RML" captures 
the effect for RML states with an RML border 

	Actually, not quite always for "RML + Border State RML", as 
	"Border State RML" can be turned on for RML states before they 
	have their own RML

*/
*c1 
reghdfe rate_total_cannabis_1899 $lea $mml_mdl $police_econ $drug_pol $sw_pol rml rml_state_border [pw=pop_1899] if race2=="white" & ever_no_rml_with_border==0, a(fips year) vce(cl fips)
	sum rate_total_cannabis_1899 [aw=pop_1899] if rml==0 & ever_rml==1 & race2=="white"
	estadd scalar pdvm = r(mean)
	est store c1 
	
*c2 
reghdfe rate_total_cannabis_1899 $lea $mml_mdl $police_econ $drug_pol $sw_pol rml rml_state_border [pw=pop_1899] if race2=="black" & ever_no_rml_with_border==0, a(fips year) vce(cl fips)
	sum rate_total_cannabis_1899 [aw=pop_1899] if rml==0 & ever_rml==1 & race2=="black"
	estadd scalar pdvm = r(mean)
	est store c2 	
	
*c3 
reghdfe rate_total_nonmj_1899 $lea $mml_mdl $police_econ $drug_pol $sw_pol rml rml_state_border [pw=pop_1899] if race2=="white" & ever_no_rml_with_border==0, a(fips year) vce(cl fips)
	sum rate_total_nonmj_1899 [aw=pop_1899] if rml==0 & ever_rml==1 & race2=="white"
	estadd scalar pdvm = r(mean)
	est store c3 
	
*c4 
reghdfe rate_total_nonmj_1899 $lea $mml_mdl $police_econ $drug_pol $sw_pol rml rml_state_border [pw=pop_1899] if race2=="black" & ever_no_rml_with_border==0, a(fips year) vce(cl fips)
	sum rate_total_nonmj_1899 [aw=pop_1899] if rml==0 & ever_rml==1 & race2=="black"
	estadd scalar pdvm = r(mean)
	est store c4 	

*c5 
reghdfe rate_property_1899 $lea $mml_mdl $police_econ $drug_pol $sw_pol rml rml_state_border [pw=pop_1899] if race2=="white" & ever_no_rml_with_border==0, a(fips year) vce(cl fips)
	sum rate_property_1899 [aw=pop_1899] if rml==0 & ever_rml==1 & race2=="white"
	estadd scalar pdvm = r(mean)
	est store c5 
	
*c6 
reghdfe rate_property_1899 $lea $mml_mdl $police_econ $drug_pol $sw_pol rml rml_state_border [pw=pop_1899] if race2=="black" & ever_no_rml_with_border==0, a(fips year) vce(cl fips)
	sum rate_property_1899 [aw=pop_1899] if rml==0 & ever_rml==1 & race2=="black"
	estadd scalar pdvm = r(mean)
	est store c6	
	
*c7 
reghdfe rate_violent_1899 $lea $mml_mdl $police_econ $drug_pol $sw_pol rml rml_state_border [pw=pop_1899] if race2=="white" & ever_no_rml_with_border==0, a(fips year) vce(cl fips)
	sum rate_violent_1899 [aw=pop_1899] if rml==0 & ever_rml==1 & race2=="white"
	estadd scalar pdvm = r(mean)
	est store c7 
	
*c8 
reghdfe rate_violent_1899 $lea $mml_mdl $police_econ $drug_pol $sw_pol rml rml_state_border [pw=pop_1899] if race2=="black" & ever_no_rml_with_border==0, a(fips year) vce(cl fips)
	sum rate_violent_1899 [aw=pop_1899] if rml==0 & ever_rml==1 & race2=="black"
	estadd scalar pdvm = r(mean)
	est store c8
	
esttab c1 c2 c3 c4 c5 c6 c7 c8 using "new table 16.csv", label ///
		title("Table 16 - Panel 4 ")  ///
		keep(rml rml_state_border) ///
		noobs scalars("pdvm Pre-treat DV mean" "N N") bfmt(2) sefmt(2) sfmt(2 0) star(* 0.10 ** 0.05 *** 0.01) ///
		mtitles("Marijuana" "" "Non-Marijuana" "" "Property" "" "Violent" "") nogaps se nonotes ///
		page append 
...
keep state state_abb fips year race race2 num_agencies_report pop_1899 rate_total_cannabis_1899 rate_property_1899 rate_violent_1899 rate_total_nonmj_1899 no_rml_with_border mml decrim unemployment lnpci

gen temp=year if no_rml_with_border>0 & no_rml_with_border!=.
egen g_border_rml=min(temp), by(fips)
replace g_border_rml=0 if g_border_rml==.
drop temp 
...
order rate_total_cannabis_1899 rate_total_nonmj_1899 rate_total_property_1899 rate_total_violent_1899 pop_1899, after(race)

preserve 

keep if race2=="black"
saveold "$path\analysis\exploratory_output\rml_black2.dta", replace

restore 

keep if race2=="white"
saveold "$path\analysis\exploratory_output\rml_white2.dta", replace


****************
**# CS event studies
****************
*see: "C:\Users\Zachary.Fone\Dropbox\RESEARCH\RML project\empirical\analysis\exploratory_programs\no rml-rml border event studies.R"

clear 

cd "$path\analysis\exploratory_output\tables 16-17a-ap2\table 16\"
...
**Total marijuana 
	...
	graph export "CS_no rml-rml border_mj_tot.png", replace

**Total non-marijuana 
	use "total_nonmj.dta", clear 
	...
	twoway (rcap conflow confhigh eventtime if race=="white", lcolor(navy) lwidth(thin)) (connected estimate eventtime if race=="white", lcolor(navy) mcolor(navy) msize(small))  || (rcap conflow confhigh eventtime if race=="black", lcolor(maroon) lwidth(thin)) (connected estimate eventtime if race=="black", lcolor(maroon) mcolor(maroon) msize(small)), yline(0, lcolor(gs8)) xline(-0.5, lcolor(gs8)) xlabel(-5(1)3) ytitle(Coefficient Estimate, size(small)) xtitle(Years Relative to RML Enactment, size(small)) graphregion(color(white)) legend(order(2 4) label(2 "White") label(4 "Black")) ylabel(-500(250)500)
	graph export "CS_no rml-rml border_non-mj_tot.png", replace
...
**Property
	...
	twoway (rcap conflow confhigh eventtime if race=="white", lcolor(navy) lwidth(thin)) (connected estimate eventtime if race=="white", lcolor(navy) mcolor(navy) msize(small))  || (rcap conflow confhigh eventtime if race=="black", lcolor(maroon) lwidth(thin)) (connected estimate eventtime if race=="black", lcolor(maroon) mcolor(maroon) msize(small)), yline(0, lcolor(gs8)) xline(-0.5, lcolor(gs8)) xlabel(-5(1)3) ytitle(Coefficient Estimate, size(small)) xtitle(Years Relative to RML Enactment, size(small)) graphregion(color(white)) legend(order(2 4) label(2 "White") label(4 "Black")) ylabel(-1000(250)750)
	graph export "CS_no rml-rml border_property.png", replace
...
**Violent 
	...
	twoway (rcap conflow confhigh eventtime if race=="white", lcolor(navy) lwidth(thin)) (connected estimate eventtime if race=="white", lcolor(navy) mcolor(navy) msize(small))  || (rcap conflow confhigh eventtime if race=="black", lcolor(maroon) lwidth(thin)) (connected estimate eventtime if race=="black", lcolor(maroon) mcolor(maroon) msize(small)), yline(0, lcolor(gs8)) xline(-0.5, lcolor(gs8)) xlabel(-5(1)3) ytitle(Coefficient Estimate, size(small)) xtitle(Years Relative to RML Enactment, size(small)) graphregion(color(white)) legend(order(2 4) label(2 "White") label(4 "Black")) ylabel(-400(200)400)
	graph export "CS_no rml-rml border_violent.png", replace	
