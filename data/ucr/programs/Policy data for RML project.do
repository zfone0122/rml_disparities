...
**# Clean data
******
import delimited using "$path\data\ucr\data\policy\RML_Control.csv", varnames(1)
...
save "$path\data\ucr\data\policy\rml-mml_2000-2019.dta", replace
restore 
...
save "$path\data\ucr\data\policy\covariates_2000-2019.dta", replace
...
foreach v in rml_1 {
forval f = 6(-1)1 { //leads
sort state_fips year
gen F`f'_`v' = F`f'.`v'
gsort state_fips -year
if `f' == 6 bys state_fips: gen sum_F`f'_`v' = sum(F`f'_`v')
}

forval l = 0/4  { //lags
sort state_fips year
qui gen L`l'_`v' = L`l'.`v'
if `l' == 4 bys state_fips: gen sum_L`l'_`v' = sum(L`l'_`v')
}

replace F6_`v' = sum_F6_`v' if F6_`v' != .
...
*save 
save "$path\data\ucr\data\policy\rml events_2000-2019.dta", replace  
...
*save 
save "$path\data\ucr\data\policy\rml-sales_year-month_2000-2019.dta", replace 
...
*save 
save "$path\data\ucr\data\policy\rml-sales_year_2000-2019.dta", replace 
