...
**total state population (based on agencies) - prior to dropping any agencies
egen tot_pop=sum(population), by(fips_state_code year)
...
gen num_agencies=1
collapse (sum) agg_assault_1899_all-total_1899_white num_agencies population pop_r_all pop_r_black pop_r_white (mean) number_of_months_reported-num_months_arson num_months_liquor num_months_drunkenness num_months_total_drug-num_months_disorder_cond, by(year state state_abb fips tot_pop)
keep if inrange(fips, 1, 56)
sort fips year
gen report_share_ucr=population/tot_pop 


**save 
save "$path\data\ucr\data\ucr\cleaned\ucr_state-year_2000-2019_cleaned.dta", replace 
