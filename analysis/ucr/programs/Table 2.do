...
foreach i in black white {
	...
	*c1
	reghdfe rate_poss_cannabis_1899 $lea $mml_mdl $police_econ $drug_pol $sw_pol rml [pw=pop_1899] if race2=="`i'", a(fips year) vce(cl fips)
		sum rate_poss_cannabis_1899 [aw=pop_1899] if rml==0 & ever_rml==1 & race2=="`i'"
		estadd scalar pdvm = r(mean)
		est store c1
		
	*c2
	reghdfe rate_sale_cannabis_1899 $lea $mml_mdl $police_econ $drug_pol $sw_pol rml [pw=pop_1899] if race2=="`i'", a(fips year) vce(cl fips)
		sum rate_sale_cannabis_1899 [aw=pop_1899] if rml==0 & ever_rml==1 & race2=="`i'"
		estadd scalar pdvm = r(mean)
		est store c2		
		
	esttab c1 c2 using "Table 2.csv", label ///
		title("Table 2 - `i' ")  ///
		keep(rml) mtitle("MJ-Poss" "MJ-Sale") ///
...
**difference in coefficients
	*c1
	reghdfe rate_poss_cannabis_1899 $lea_interx $mml_mdl_interx $police_econ_interx $drug_pol_interx $sw_pol_interx rml rml_black [pw=pop_1899] if race!=1, a(i.fips#i.black i.year#i.black) vce(cl fips)
		est store c1
		
	*c2
	reghdfe rate_sale_cannabis_1899 $lea_interx $mml_mdl_interx $police_econ_interx $drug_pol_interx $sw_pol_interx rml rml_black [pw=pop_1899] if race!=1, a(i.fips#i.black i.year#i.black) vce(cl fips)
		est store c2		
		
	esttab c1 c2 using "Table 2.csv", label ///
		title("Table 2 - diff in RML coeff. ")  ///
		keep(rml_black) ///
