...
*description: Effect of RMLs on Drinking and Delinquency-Related Part II Arrests
**********************************************************
...
foreach i in black white {
	...
	*c1
	reghdfe rate_vandalism_1899 $lea $mml_mdl $police_econ $drug_pol $sw_pol rml [pw=pop_1899] if race2=="`i'", a(fips year) vce(cl fips)
		sum rate_vandalism_1899 [aw=pop_1899] if rml==0 & ever_rml==1 & race2=="`i'"
		estadd scalar pdvm = r(mean)
		est store c1
		
	*c2
	reghdfe rate_liquor_1899 $lea $mml_mdl $police_econ $drug_pol $sw_pol rml [pw=pop_1899] if race2=="`i'", a(fips year) vce(cl fips)
		sum rate_liquor_1899 [aw=pop_1899] if rml==0 & ever_rml==1 & race2=="`i'"
		estadd scalar pdvm = r(mean)
		est store c2	
		
	*c3
	reghdfe rate_dui_1899 $lea $mml_mdl $police_econ $drug_pol $sw_pol rml [pw=pop_1899] if race2=="`i'", a(fips year) vce(cl fips)
		sum rate_dui_1899 [aw=pop_1899] if rml==0 & ever_rml==1 & race2=="`i'"
		estadd scalar pdvm = r(mean)
		est store c3	
		
	*c4
	reghdfe rate_drunk_dconduct_1899 $lea $mml_mdl $police_econ $drug_pol $sw_pol rml [pw=pop_1899] if race2=="`i'", a(fips year) vce(cl fips)
		sum rate_drunk_dconduct_1899 [aw=pop_1899] if rml==0 & ever_rml==1 & race2=="`i'"
		estadd scalar pdvm = r(mean)
		est store c4

		
	esttab c1 c2 c3 c4 using "Appendix Table 5.csv", label ///
		title("Appendix Table 5 - `i' ")  ///
		keep(rml) mtitle("Vandalism" "Liquor Law Violations" "DUI" "Drunkenness/Disorderly Conduct") ///
...
**difference in coefficients
	*c1
	reghdfe rate_vandalism_1899 $lea_interx $mml_mdl_interx $police_econ_interx $drug_pol_interx $sw_pol_interx rml rml_black [pw=pop_1899] if race!=1, a(i.fips#i.black i.year#i.black) vce(cl fips)
		est store c1
		
	*c2
	reghdfe rate_liquor_1899 $lea_interx $mml_mdl_interx $police_econ_interx $drug_pol_interx $sw_pol_interx rml rml_black [pw=pop_1899] if race!=1, a(i.fips#i.black i.year#i.black) vce(cl fips)
		est store c2	
		
	*c3
	reghdfe rate_dui_1899 $lea_interx $mml_mdl_interx $police_econ_interx $drug_pol_interx $sw_pol_interx rml rml_black [pw=pop_1899] if race!=1, a(i.fips#i.black i.year#i.black) vce(cl fips)
		est store c3
		
	*c4
	reghdfe rate_drunk_dconduct_1899 $lea_interx $mml_mdl_interx $police_econ_interx $drug_pol_interx $sw_pol_interx rml rml_black [pw=pop_1899] if race!=1, a(i.fips#i.black i.year#i.black) vce(cl fips)
		est store c4

		
	esttab c1 c2 c3 c4 using "Appendix Table 5.csv", label ///
		title("Appendix Table 5 - diff in RML coeff. ")  ///
		keep(rml_black) ///
		mtitle("Vandalism" "Liquor Law Violations" "DUI" "Drunkenness/Disorderly Conduct") ///
